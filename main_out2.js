(function (wHandle, wjQuery) {
    ...
    wHandle.setserver = function (arg) {
              if (arg != gameMode) {
                  ...
                  gameMode = arg;
                  showConnecting();
                  var url = "updateserver/" + arg;
                  $.ajax({url: url,});
              }
    };

	var emDateN = new Date();
    var flood_time = "5500"; // 5500 = 5 segs (1 message every 5 segs)
    var chat_msg_flod = "تم اكتشاف محاولة سبام يرجى الانتظار 5 ثواني";
    var last, diff;
    var myx = 0;
    var myy = 0;
    ...
    function drawChatBoard() {
        ...
        var scaleFactor = 1;//Math.min(Math.max(canvasWidth / 1200, 0.75),1); //scale factor = 0.75 to 1
        ...
        for (var i = 0; i < (len - from); i++) {
            ...
            chatName.setValue(chatBoard[i + from].name.trim());
            ...
            chatText.setValue(' : ' + chatBoard[i + from].message.trim());
            a = chatText.render();
    ...
    function sendChat(str) {
        if ( last > 0 ) {
            diff = wjQuery.now() - last;
        }
        var floodis = $("#autohidecflo").length;
        if(diff < flood_time){
            if(floodis == 0){
                $("body").append('<div id="autohidecflo" style="display:none;position: absolute;left: 50%;top: 30%;transform: translate(-50%, -50%);max-width: 50%;text-align: center;color: red;font-size: 25px;"><b>'+chat_msg_flod+'</b></div>');
            }else{
                $("#autohidecflo").show('fast').delay(6000).hide('fast');
            }
        }else{
        ...
        if (wsIsOpen() && (str.length < 200) && (str.length > 0)) {
            ...
            wsSend(msg);
            //console.log(msg);
		 /*setHideChat('lastchat',new Date().getTime(),999);*/
    ...
    function drawGameScene() {
        ...
        if (!hideChat)
        {
            if ((chatCanvas != null)&&(chatCanvas.width > 0)) ctx.drawImage(chatCanvas, 0, canvasHeight - chatCanvas.height - 50); // draw Leader Board
        }
...
// map -->
		...
		ctx.fillText(Math.round(nodeX/1000)+' , '+Math.round(nodeY/1000), 60, 170);
		// <--
